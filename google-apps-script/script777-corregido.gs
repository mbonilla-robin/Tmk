...
function doPost(e) {
  try {
    ...
    if (idTarea !== "") {
      if (lastRow >= 1) {
        var ids = targetSheet.getRange(1, 9, lastRow, 1).getValues();
        for (var i = 0; i < ids.length; i++) {
          if (String(ids[i][0]).trim() === idTarea) {
            targetRow = i + 1;
            break;
    ...
    if (targetRow === -1 && (payload.info || payload.originalInfo) && lastRow >= 3 && marca !== "Config_Marcas") {
      var brandData = targetSheet.getRange(3, 1, lastRow - 2, 9).getValues();
      var valCat = String(payload.categoria || "").trim().toLowerCase();
      var valInfo = String(payload.info || "").trim().toLowerCase();
      var valInfoOriginal = String(payload.originalInfo || "").trim().toLowerCase();

      for (var j = 0; j < brandData.length; j++) {
        var filaCat = String(brandData[j][1]).trim().toLowerCase();
        var filaInfo = String(brandData[j][3]).trim().toLowerCase();
        var catCoincide = filaCat === valCat;
        var infoCoincide = filaInfo === valInfo || (valInfoOriginal !== "" && filaInfo === valInfoOriginal);
        if (catCoincide && infoCoincide) {
          targetRow = j + 3;
          break;
        }
      }

      if (targetRow === -1 && valInfoOriginal !== "") {
        for (var k = 0; k < brandData.length; k++) {
          if (String(brandData[k][3]).trim().toLowerCase() === valInfoOriginal) {
            targetRow = k + 3;
            break;
          }
        }
