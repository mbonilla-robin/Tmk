...
function MasOpcionesMenu({ onEstatus, variant = "sidebar" }) {
      ...
      <div className="flex flex-col gap-0.5" ref={ref}>
        <button
          ...
          className={`flex items-center gap-2 w-full px-2 py-1.5 rounded text-ui font-medium transition-all ${
            abierto ? "bg-white shadow-sm text-zinc-900 border border-zinc-200" : "text-zinc-600 hover:bg-zinc-100/50"
          }`}
        >
          <i className="fa-solid fa-ellipsis text-[10px] text-zinc-400"></i>
          <span>Más opciones</span>
          <i className={`fa-solid fa-chevron-down text-[8px] text-zinc-400 ml-auto transition-transform duration-150 ${abierto ? "rotate-180" : ""}`}></i>
        </button>
        {abierto && (
          <button
            type="button"
            onClick={() => { setAbierto(false); onEstatus(); }}
            className="flex items-center gap-2 w-full pl-6 pr-2 py-1.5 rounded text-ui font-medium text-zinc-600 hover:bg-zinc-100/50 transition-all"
          >
            <SVGIcon.FileText className="w-3 h-3 text-zinc-400" />
            <span>Estatus</span>
          </button>
        )}
      </div>
